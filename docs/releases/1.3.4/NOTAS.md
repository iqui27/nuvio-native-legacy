...
- **Resume sometimes opened the source list instead of resuming** (rawldon on the rc1). With "Choose the source when playing" on, the remembered pick had to match today's list exactly by add-on and language marks; the next episode from the same add-on often carries one extra mark in its label ("Dual Audio" today, "BR DUB" tomorrow) and the match failed, so the sheet came back. A pick now also matches when today's marks contain the remembered ones. A pick with a mark that is missing today still does not match — DUB never falls back to the original audio.
- **Some streams took long to open, especially the first time** (Samsung). The rc1 had cut the add-on queries to two at a time; with six add-ons and one of them hanging for 25 s the list only closed on the third round. Back to four in parallel — the freeze cause was the image decoding, not these threads, and the rc1 proved it.
- **GIF artwork stuttered on Samsung** (rawldon on the rc1). Only the next frame was being decoded ahead, and on the TV one frame's decode takes longer than the GIF's frame delay. Four frames are kept decoded now, and the frame clock is anchored to the previous deadline so a late frame does not push every following one.
