...
// desta tela. Ver o cabecalho de fil_aceita_tipo em fileiras.h.
static int formaFixa(const char *chave) {
  if (!chave || !chave[0]) return 1;
  if (!strcmp(chave, "continue_watching")) return 1;
  if (!strcmp(chave, "social_activity")) return 1;
  if (!strcmp(chave, "last_session")) return 1;
  return !strncmp(chave, "collection_", 11);
}
