...
void recenviar_desenhar(Uint32 agora) {
  float a = anim_suave(anim), alt, x, y, cab;
  int i, n, vis;
  ...
  else if (pagina == RE_PAG_CONTATOS && nCtts == 0) cab = 120.0f + 116.0f;
  else                                  cab = 120.0f;
  ...
  { TxtLinha t = txt_linha(TXT_CAPTION2, chapeu, 174, 178, 188, 255);
    txt_desenhar_alpha(t, x + RE_PAD, y + RE_PAD, a * 0.95f); }
  { TxtLinha t = txt_linha_corta(TXT_HEADLINE, titulo, 245, 248, 255, 255,
                                 RE_INTERNO);
    txt_desenhar_alpha(t, x + RE_PAD, y + RE_PAD + 28.0f, a); }
  { TxtLinha t = txt_linha(TXT_DET_META2, pergunta, 150, 154, 163, 255);
    txt_desenhar_alpha(t, x + RE_PAD, y + RE_PAD + 74.0f, a * 0.9f); }
  ...
  if (pagina == RE_PAG_AMIGOS) {
    ...
    TxtLinha t1 = txt_linha(TXT_CALLOUT, "Nenhum amigo ainda",
                            240, 242, 248, 255);
    txt_desenhar_alpha(t1, x + RE_PAD, y + RE_PAD + 106.0f, a * 0.96f);
    txt_bloco(TXT_CAPTION,
        ...
        190, 194, 204, x + RE_PAD, y + RE_PAD + 146.0f, RE_INTERNO, 30.0f,
        a * 0.9f, 2);
  }
