...
typedef struct {
  ...
  const char *icone;            // SEC: basename em art/icones
} Item;
#define SEC(t, s, ic) { IT_SEC, -1, t, s, ic }
#define GRP(t, s)     { IT_GRP, -1, t, s, NULL }
...
// Os icones sao os de art/icones; nao existe paleta nem "info" la, entao
// Aparencia usa o de aspecto e Sobre o de confirmacao.
static const Item TELA[] = {
  SEC("Conta", "Conta e status de sincronização", "menu_profile"),
    OPC(AJ_PERFIL_ATIVO), OPC(AJ_SYNC), OPC(AJ_SAIR),

  SEC("Aparência", "Cor de destaque, idioma e animações", "aspecto"),
    OPC(AJ_TEMA), OPC(AJ_IDIOMA), OPC(AJ_ANIM),

  SEC("Layout", "Estrutura da página inicial e estilos de pôster", "menu_home"),
    GRP("Layout da Home", "Escolha a estrutura e a fonte do destaque."),
      OPC(AJ_LANDSCAPE), OPC(AJ_HERO_CHEIO), OPC(AJ_HERO_FUNDO),
      OPC(AJ_HERO_ARTE_DIF), OPC(AJ_HERO_TRAILER),
    GRP("Conteúdo da Home", "Controle o que aparece na home e na busca."),
      ...
      OPC(AJ_PS_FUNDO),
    GRP("Continuar assistindo", "Configure os próximos episódios e a ordem."),
    ...
    GRP("Página de detalhes", "Personalize as telas de títulos e episódios."),
    ...
    GRP("Foco no pôster", "Defina o comportamento ao selecionar um título."),
      OPC(AJ_EXPANDIR), OPC(AJ_EXPANDIR_ATRASO), OPC(AJ_BORDA_FOCO),
    GRP("Estilo dos cartões", "Largura, arredondamento e efeito de profundidade."),
  ...
  SEC("Conteúdo", "Addons e portais de TV ao vivo", "addon"),
    OPC(AJ_ADDONS),
    ROT("Portal Stalker"),
  ...
  SEC("Integrações", "Serviços de metadados e de notas", "fluxo"),
    GRP("TMDB", "Metadados, arte, elenco e trailers vindos do TMDB."),
    ...
    GRP("MDBList", "Quais fontes de nota aparecem na página do título."),
      ...
      OPC(AJ_MDB_MAL),
    GRP("fanart.tv", "Chave pessoal para a arte do destaque."),
      OPC(AJ_FANART_CHAVE),
  ...
  // de um OK a mais.
  SEC("Reprodução", "Player, fontes, áudio e legendas", "play"),
  ...
  SEC("Trakt e Simkl", "Histórico, progresso e listas", "visto"),
  ...
  SEC("Avançado", "Desempenho, navegação, cache e diagnósticos", "menu_settings"),
    OPC(AJ_NAV_RAPIDA), OPC(AJ_RESOLUCAO), OPC(AJ_QUALIDADE_IMG),
    OPC(AJ_TEX_MB), OPC(AJ_ESPACO), OPC(AJ_DIAGNOSTICO),

  SEC("Sobre", "Versão, atualizações e registros", "check"),
...
static const char *SECAO_AJUDA[] = {
  ...
  "Ajustes desta TV: navegação, resolução, memória para imagens e o diagnóstico.",
...
// coluna de categorias e no painel da direita.
